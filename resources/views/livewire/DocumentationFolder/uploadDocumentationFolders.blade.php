        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li class="inline-flex items-center">
                <div class="flex items-center">
                    ...
                    <a href="/documentation" class="md:ms-2 inline-flex items-center text-sm font-medium text-gray-700 hover:text-[#C0A16E]">
                ...
                <div class="grid">
                    ...
                    @else
                        <div id="accordion-collapse" data-accordion="collapse">
                            <ul wire:sortable="updateDocumentationOrder">

                                @foreach($this->documentation as $key => $documentation)

                                    <li wire:sortable.item="{{$documentation->id}}" wire:key="{{$key}}" >
                                                    ...
                                                    <div class="p-5 border border-gray-200 dark:border-gray-700">
                                                        <div class="relative z-0 w-full mb-5 group">
                                                            <div class="text-right">
                                                                <i wire:click="removeDocumentation({{$documentation->id}})" class="fa-solid fa-trash hover:cursor-pointer"></i>
                                                            </div>
                                                            <label for="project_name" class="text-gray-400">{{ __('messages.Bestandsnaam') }}</label>
                                                            <input placeholder="{{$documentation->friendly_name}}" type="text" wire:model="friendly_name.{{$documentation->id}}"  name="friendly_name.{{$documentation->id}}" id="friendly_name.{{$documentation->id}}" class="block py-2.5 px-0 w-full text-md text-gray-900 border-0 border-b-2 border-gray-300 appearance-none dark:text-gray-900 dark:border-gray-600 focus:outline-none focus:ring-0 focus:border-b-[#C0A16E]"  required />
                                                            <div class="text-red-500">@error('friendly_name.'.$documentation->id) {{ $message }} @enderror</div>

                                                            <div class="text-right">
                                                                <br/>
                                                                <button wire:click="updateFileName({{$documentation->id}})" type="button" class="disabled:cursor-not-allowed w-[100%] md:w-auto text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">
                                                                    Updaten
                                                                </button>
                                                            </div>
                                                        </div>
